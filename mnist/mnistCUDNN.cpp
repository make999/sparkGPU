...
{
    sFilename = (std::string("/home/make/finaljc/mnist/data/") + std::string(fname));
}
...
class network_t
{
  ...
  public:
    ...
    int classify_example(int img[], const Layer_t<value_type>& conv1,
                          ...
                          const Layer_t<value_type>& ip2)
    {
		...
		for(int i = 0; i < IMAGE_H*IMAGE_W; i++) {
			imgData_h[i] = (double)(img[i])/(double(255));
			printf(" %d ", img[i]);
		}
...
int myext(int img[], int argc, const char *argv[])
{   
    std::string image_path;
    int i1,i2,i3;

	printf("THIS FROM JAVA length = %d\n", argc);
...
//        getCmdLineArgumentString(argc, (const char **)argv,
  //                               "image", (char **) &image_name);        
        ...
        i1 = mnist.classify_example(img, conv1, conv2, ip1, ip2);
...
extern "C" 
void sendimg(int *in, int sz) {

	const char **arg;
	arg[1] = "device=1";
	//arg[2] = "image=tt.jpg";
	int a = myext(in, sz, arg);
